...
import { useCategories, useInstallments, useRecurringBills } from '../hooks/useFirestore';
...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const monthlyInstallments = activeInstallments.reduce((s, i) => s + i.monthlyAmount, 0);
    ...
    const breakdown = Array.from(catMap.entries())
      .map(([categoryId, total]) => {
        ...
        return { categoryId, total, name: cat?.name || 'Ostalo', color: cat?.color || '#64748b', icon: cat?.icon || 'Circle' };
      })
    ...
    return { income, expenses, balance: income - expenses, monthlyInstallments, monthlyBills, breakdown, dailyData };
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <motion.div variants={itemVariants}>
          <Card className="gradient-mesh">
            ...
            <p className={cn('text-xl sm:text-2xl font-black', stats.balance >= 0 ? 'text-accent-400' : 'text-danger-400')}>
        ...
        <motion.div variants={itemVariants}>
          <Card>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-medium opacity-60">Rate + Racuni</span>
            </div>
            <p className="text-xl sm:text-2xl font-black text-warning-400">
              {formatCurrency(stats.monthlyInstallments + stats.monthlyBills, currency)}
            </p>
          </Card>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2">
          <Card>
            <h3 className="text-sm font-bold mb-4 opacity-80">Dnevna potrosnja</h3>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={stats.dailyData}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [formatCurrency(Number(value), currency), 'Potrosnja']}
                  />
                  <Area type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={2.5} fill="url(#colorAmount)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-[220px] flex items-center justify-center opacity-40 text-sm">
        ...
        <motion.div variants={itemVariants}>
          <Card>
            <h3 className="text-sm font-bold mb-4 opacity-80">Po kategorijama</h3>
            {stats.breakdown.length > 0 ? (
              ...
              <div className="h-[180px] flex items-center justify-center opacity-40 text-sm">
                Nema rashoda
              </div>
            )}
          </Card>
      ...
      <motion.div variants={itemVariants}>
        <Card>
          ...
          {transactions.length > 0 ? (
              ...
              {transactions.slice(0, 8).map((t) => {
                ...
                return (
                  <div key={t.id} className={cn(
                    ...
                    theme === 'dark' ? 'hover:bg-dark-800/60' : 'hover:bg-dark-50'
                  )}>
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                      style={{ backgroundColor: (cat?.color || '#64748b') + '20' }}
                    >
                      <Icon name={cat?.icon || 'Circle'} size={18} style={{ color: cat?.color || '#64748b' }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate">{t.description}</p>
                      <p className="text-xs opacity-50">{cat?.name} &middot; {format(new Date(t.date), 'dd. MMM')}</p>
                    </div>
                    <div className={cn('flex items-center gap-1 text-sm font-bold', t.type === 'income' ? 'text-accent-400' : 'text-danger-400')}>
            ...
            <EmptyState icon="ArrowLeftRight" title="Nema transakcija" description="Dodaj prvu transakciju za ovaj mjesec" />
          )}
        </Card>
      </motion.div>
    </motion.div>
